...
Designed, not built, and the design has an open question at its centre — what a
...
coarsening has to happen in the view.

**The complaint is arithmetic, not taste.** `ecoli_pggb_is5.gfa` is 20 segments
/ 26 links / 5 paths over 1,414 bp, and twelve of the twenty segments are 1 bp.
The figure runs `bubbleSpread: 'open'`, whose floor is `2.5 * MEAN_NODE_LENGTH`
= 100 FMMM units, and `bandageAutoScale` puts this graph at 0.566 units/bp — so
...
twelve 1 bp alleles hold **47% while carrying 0.8% of the sequence**, and the
arm the figure is about holds 26% while carrying 85%.
...
Both were rendered and rejected; what each of the four spreads is an instrument
for is jbrowse-components' `website/docs/user_guides/graph_genome_view.md`, and
the review notes are in its `0c432e1141`. The fourth, `wide`, is untried here
and is not worth trying: it is `open`'s floor lifted higher, so it clamps the
same nineteen nodes.

**Collapse the bubble, and that is what lets the floor come off.** The two are
one change: the floor exists only to give a bubble's ARMS room to separate, and
a collapsed bubble has no arms. With both, this graph is 13 nodes at 0.368
...
- **Path lanes are the open question, and why this figure is the test case.**
  ...
  suppressing collapsing while `drawPaths` is on, leaves this figure as it is
  and buys nothing.
- **Expand on click.** For a file-loaded graph the arms are already in memory,
  so it is view state rather than a fetch — cheaper here than on the tier route.

Two findings already paid for, so they are not re-priced: **chain contraction is
the wrong primitive** (`vg mod -u` measured at 0.95% on HPRC chr20, because at
