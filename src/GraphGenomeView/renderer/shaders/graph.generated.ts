...
//
// Both of those lines are aspirational: neither the codegen nor graph.slang is
// in this repo (see createGraphRenderer), there is no `gen:shaders` script, and
// this file is the only copy. So it gets hand-edited when render-core moves
// under it, and 2026-08-13 was such a move — 4.3.0 renamed the type to
// VertexAttributeLayout, which broke `pnpm typecheck` outright. The constant is
// renamed to match, because VERTEX_ATTRIBUTES is what the current codegen emits
// and what slangPass()'s SlangShaderModule requires: the GPU backend that
// eventually runs this module's WGSL will want it spelled that way.

import type { VertexAttributeLayout } from '@jbrowse/render-core/hal'
