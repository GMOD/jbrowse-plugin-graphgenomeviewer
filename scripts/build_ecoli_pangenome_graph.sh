...
# ── Local subgraph for the Graph genome view (Bandage-style 2-D layout) ───────
# The whole graph is far too large to force-direct, so cut one window out of it:
# `odgi extract -E` takes every node between the first and last in the range,
# `odgi sort -O` compacts the node ids, `odgi view -g` writes GFA. This window
# straddles a coverage step (two strains on one side, four on the other), so the
# extracted graph is one backbone plus the bubbles where strains diverge. 400 bp
# is a legibility limit, not a capability one: the layout keeps a bubble's two
# alleles a fixed fraction of a chain step apart, so a 10 kb window zoomed to
# fit renders its bubbles smaller than the ribbons drawn over them.
...
# the subgraph is opened by URL in a Graph genome view, not added as a track, so
# it just needs to sit beside config.json
cp ecoli_pggb_subgraph.gfa "$APP/"
